...
def dplr(
    ...
    if init in ['random', 'rand']:
        ...
        real_part = torch.cos((2*imag_part+1)*pi/(2*N))
        imag_part = torch.sin((2*imag_part+1)*pi/(2*N))
    else: raise NotImplementedError
...
def ssm(init, N, R, H, **ssm_args):
    ...
    elif init.startswith("filter"):
        ssm_args["P_scale"] = 0.0
        A, P, B, V = dplr(init, N=N, rank=R, H=H, **ssm_args)
    else:
...
class Kernel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        lr: Optional tuple with default value and dictionary specifying special hyperparameters for .register().
        ...
        if self.lr is None or isinstance(self.lr, float):
            ...
            self.lr_dict = defaultdict(lambda: self.lr[0])
            self.lr_dict.update(self.lr[1])
...
class SSMKernel(Kernel):
    ...
    def init_ssm_dplr(self):
        ...
        # rotation = X[:,0,:][:,None].conj()
        # X = X * rotation / abs(rotation)
        # C = X.permute((2,0,1)) / B

        # # Normalize C such that ||B*C|| = 1
        # C /= torch.norm(B * C, dim=-1, keepdim=True)
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        if self.feedback:
            assert self.rank == self.channels
