...
class SSMKernelDiag(SSMKernel):
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        # If use S4DC, store C=BC instead of C
        if self.lr_dict['C'] == 0.0 and self.power_iter:
            C = B * C
    ...
    def _get_params(self, rate=1.0):
        ...
        if self.lr_dict['C'] == 0.0 and self.power_iter:
            M = -1/(A[:,:,None] + A[:,None,:].conj()) # (H N N)
            M = M - torch.norm(M,dim=(1,2),keepdim=True)
            
            for _ in range(self.power_iter):
                C = torch.einsum('bij,abj->abi', M, C)
                C = F.normalize(C, dim=2)
            self.C = _c2r(C).detach() # update self.C = BC
            C = C / B

            # X = B * C # torch.einsum('cbi,abi->abi', B, C) # (C H N)
            # for _ in range(self.power_iter):
            #     X = torch.einsum('bij,abj->abi', M, X)
